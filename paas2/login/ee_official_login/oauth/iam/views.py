...
def login(request):
    ...
    # 若没有code参数，则表示需要跳转到iam登录
    if code is None or is_from_logout:
        # 生成跳转到iam登录的链接
        iam_oauth_login_url, state = gen_oauth_login_url({"app_id": app_id, REDIRECT_FIELD_NAME: redirect_to})
        # 将state 设置于session，Oauth2.0特有的，防止csrf攻击的
        request.session["state"] = state
        # 直接调用蓝鲸登录重定向方法
        response = actions.login_redirect_response(request, iam_oauth_login_url, is_from_logout)
        logger.debug(
            "custom_login:oauth.iam code is None or is_from_logout! code=%s, is_from_logout=%s",
            code,
            is_from_logout,
        )
        return response
    ...
    redirect_to = state_dict.get(REDIRECT_FIELD_NAME, "/console/")
    state_from_session = request.session.get("state")
    # 校验state，防止csrf攻击
    if state != state_from_session:
        logger.debug(
            "custom_login:oauth.iam state != state_from_session [state=%s, state_from_session=%s]",
            state,
            state_from_session,
        )
    ...
    # 验证用户登录是否OK
    user = OauthBackend().authenticate(code=code,request=request)
    if user is None:
        logger.debug("custom_login:oauth.iam user is None, will redirect_to=%s", redirect_to)
        # 直接调用蓝鲸登录失败处理方法
        return actions.login_failed_response(request, redirect_to, app_id)
    # 成功，则调用蓝鲸登录成功的处理函数，并返回响应
    logger.debug("custom_login:oauth.iam login success, will redirect_to=%s", redirect_to)
